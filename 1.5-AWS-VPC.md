...
## 1.5.7 VPC Security Groups (SG)

- Stateful - detect response traffic automatically
- Allowed (IN or OUT) request will in turn allow the response
- **No Explicit DENY**: requires NACL
  - Cannot block specific bad actor
- Supports IP and CIDR rules as well as logical resources including other
  security groups and itself
- Security groups are attached to Elastic Network Interface (ENI), not the
  instance itself.
- Logical referencing scales. any new instances which use the referenced SG are
  allowed to communicate with any new source SG.
- Allows self referencing its logical self which means any resources attached to
  this SG can communicate with each other.

## 1.5.8 Network Address Translation (NAT) & NAT Gateway

- What is NAT?
  - NAT allows a set of processes that remaps SRC or DST IP addresses.
- Port Address Translation (PAT) (or IP Masquerading)
  - Hide CIDR blocks behind one IP
  - Useful for conserving Public IPv4 Addresses
- NAT gives private CIDR range outgoing internet access and AWS Public services
- NAT Gateways run from a public subnet
- Uses Elastic IPs (Static IPv4 Public)
- **NEW** NATGW offer Regional which are Region Resilient
- **OLD** Zonal NATGW
  - NATGW are AZ resilient and have high availability in AZ but if AZ fails, so
    will NATGW
    - For region resilience, NATGW in each AZ and RT for each AZ with that NATGW
      as target
- NATGW are managed by AWS and scale to 100Gbps
  - Can provision more NATGW to handle more data volume
- Roughly $0.045 per hour (billed for partial hours)
- IPv6 doesn't require NAT.
...
- NAT Instance
  - Custom NAT running in provisioned EC2 instance
  - Not as resilient as NATGW.
  - NAT Instance can be cheaper at low and sometimes high volumes of data
  - NAT Instances share similar flexibility with other EC2 and allow both
    Security Groups of the instance and NACL of the subnet
