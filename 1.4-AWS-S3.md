...
## 1.4.8 AWS S3 Lifecycle Configuration

- Set of rules that control actions in S3 for buckets or groups of objects.
- All classes can transition downward (never upward) into deeper tiers, with
  exception of:
  - S3 One Zone-IA cannot transition to Glacier Instant Retrieval.
    - This is due to fact that moving down to G-IR would be considered an
      upgrade in availability, while retaining same access.
- Care is required when transitioning small objects from standard downward due
  to size minimum constraints per cost.
- Object needs to be in Standard for 30 days before transitioning to Standard-IA
  or One Zone-IA.
- Same is true for Standard-IA or One Zone-IA, needs to be 30 days before
  transition to glacier.

## 1.4.10 S3 Presigned URLs

- S3 Presigned URLs allow for access to S3 via an authenticated URL to allow
  unauthenticated access.
- It allows an application to act as a gatekeeper, permitting users to securely
  upload, download, or delete objects directly without needing AWS credentials
  or making the bucket public.
- Another use case is allowing access to files via authenticated server access.
  This works by having user access server to retrieve video, server then
  accesses S3 and creates a URL for the bucket and returns this. The user can
  then access the data directly from this presigned URL.

- You can generate url for an object you have no access to (and thus is quite
  limited in use case).
- When using URL, the permissions match the identity which generated it.
- Access denied could mean the generating ID never had access or doesn't now.
- Don't generate with a role
  - URL stops working when temporary credentials expire.

## 1.4.11 S3 Select and Glacier Select

- S3 can store HUGE objects (up to 50TB)
- You often want to retrieve the entire object.
- S3/Glacier Select allows SQL-Like statements to retrieve part of object,
  pre-filtered by S3.
- Allows CSV, JSON, Parquet, BZIP2 compression for CSV and JSON.

## 1.4.12 S3 Events

- Notification generated when events occur in a bucket. These can be delivered
  to SNS, SQS and Lambda functions.
- Can occur when object Created
  - PUT
  - POST
  - COPY
...
## 1.4.13 S3 Access Logs

- S3 Access logs allow a way to dump access logs into a target bucket.
- These logs consist of records that are newline-delimited, attributes are space
  delimited

## 1.4.14 S3 Object Lock
...
- Allows different types:
  - Retention Period
  - Legal Hold
  - Both, One or the other, or none can be set
- Buckets can have default Object Lock Settings
- A bucket can have default object lock settings

- Retention
  - Specify DAYS & YEARS - A retention period
  - Compliance mode
    - can't be adjusted, deleted, overwritten during period even by root user
      until retention expires
    - Required in strict industries such as medical or legal
  - Governance mode
    - Special permissions can be granted allowing lock settings to be adjusted.
    - e.g. specific identities can use `s3:BypassGovernanceRetention`.
...
## 1.4.15 S3 Access points

- Simplify managing access to S3 Buckets/Objects
- Rather than 1 bucket w/ 1 Bucket Policy, S3AP allows many access points each
  with different policies and different network access controls.
- Each AP has its own endpoint address
- Can be created via console or `aws s3control create-access-point --name <NAME>
  --account-id <ID> --bucket <BUCKET>`.
- Allows access to specific identities via each AP, they have their own
  policies.
- VPC can also be allowed an access point that only VPC can connect on.
  - Arch may have open wide policy on the bucket, and then granular at the VPC
    policy, filtered down to each access within the VPC.
