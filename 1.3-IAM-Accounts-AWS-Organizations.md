...
- orgs allows for consolidation of reservations and volume discounts accross
  accounts.
- `OrganizationAccountAccessRole` Default role name to switch to account.

### Service Control Policies (SCP)

Service Control Policies are policy documents that can be attached to:
1. Root org container
1. Org unit
1. Individual AWS accounts

- SCPs inherit down the hierarchy.
- Management accounts cannot have SCPs attached to them.
- SCPs can reduce the permissions of an account (account permissions boundary)
  which in turn can implicitly restrict a **root user**.
- SCPs do not grant permissions but setup what is allowed; Identity policies are
  still needed to allow permissions.

## 1.3.6 AWS CloudTrail

- Enabled by default, but 90 days, no data to s3
- Trails are how S3 and CWLogs are configured
- Certain services that are global (IAM, STS, CloudFront) log as global service
  events to us-east-1
- Not realtime: There is a delay of ~15 minutes

## 1.3.6 AWS Control Tower (More Research Required)

- Quick and easy setup of multi-account environment
- Orchestrates other AWS services to provide this functionality
- Orgs, IAM, CloudFormation, Config and more
- **Landing Zone** - multi-account enviornment (AWS org with super powers)
  - SSO/ID Fed, Centralised Logging & Auditing
...
- Generates Security OU - Log Archive & Audit Accounts (CloudTrail & Config
  Logs)
...
- Rules that allow multi-account governance.
- Range from Mandatory, Strongly Recommended or Elective
- Preventive: Stop you doing things in org (AWS ORG SCP)
  - These are Enforced or Not Enabled
  - e.g. Allow or deny regions, or dsiallow bucket policy changes
- Detective: Compliance checks (AWS CONFIG Rules)
  - Clear, in violation, or not enabled
  - e.g. detect CloudTrail enabled or EC2 Public IPv4.

### Account Factory

Automated Account Provisioning done by cloud admins or end users (with
appropriate permissions).
- Gives Admin rights to a named user when creating. Works great in circumstance
  of self service as gives tightly (but full) controlled accounts to the
  creator.
- Account & network standard configuration (great for avoiding addressing
  overlap in VPC, especially at scale).
